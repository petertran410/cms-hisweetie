...
import { useCreateRedirect } from '@/services/redirect.service';
...
const RedirectCreate = () => {
  ...
  const { mutate: createMutate, isPending } = useCreateRedirect();
  ...
  return (
    <div className="w-full md:w-[60%] lg:w-[50%] 2xl:w-[45%] mx-auto">
      <Helmet>
        <title>Tạo redirect mới | {WEBSITE_NAME}</title>
      </Helmet>

      <Form
        form={form}
        name="redirectCreateForm"
        labelCol={{ span: 24 }}
        wrapperCol={{ span: 24 }}
        onFinish={onFinish}
        autoComplete="off"
        className="mt-10"
        initialValues={{ status_code: 301, is_active: true }}
      >
        <Form.Item
        ...
        <div className="flex items-center gap-8 mt-20 justify-center">
          ...
          <Button type="primary" htmlType="submit" size="large" className="px-10" loading={isPending}>
            <span className="font-semibold">Tạo redirect</span>
          </Button>
        </div>
      </Form>
    </div>
