...
import { useCreateBlogCulture, useQueryBlogCultureDetail, useUpdateBlogCulture } from '@/services/blog-culture.service';
...
const BlogCultureCreate = () => {
  const { id } = useParams();
  const { isPending: loadingCreate, mutate: createMutate } = useCreateBlogCulture();
  const { isPending: loadingUpdate, mutate: updateMutate } = useUpdateBlogCulture(id);
  const { isLoading: loadingDetail, data: blogCultureDetail, error: errorDetail } = useQueryBlogCultureDetail(id);
  ...
  const { title, description, htmlContent, imagesUrl } = blogCultureDetail || {};
  ...
  return (
    <div className="w-full md:w-[60%] lg:w-[50%] 2xl:w-[65%] mx-auto mb-10">
      <Helmet>
        <title>
          {id ? 'Cập nhật' : 'Tạo'} bài viết văn hoá | {WEBSITE_NAME}
        </title>
      </Helmet>
      ...
      >
        ...
        <div className="flex items-center gap-8 mt-20 justify-center">
          ...
          {!isDetail ? (
            ...
            <Link to={`/blog-culture/${id}/edit`} onClick={() => window.scrollTo(0, 0)}>
...
export default BlogCultureCreate;
